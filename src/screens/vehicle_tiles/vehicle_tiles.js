...
const Component = (props) => {
    ...
    const FetchResults = async () => {
        ...
        await Api.GetVehiclesCount(query)
            .then(async (res) => {
                if (res.status) {
                ...
                if (res.status) {
                    ...
                    for (let i = 0; i < _rows.length; i++) {
                        _rows[i].id = Helper.GetGUID();
                                                                                                _rows[i].MainPhoto &&
                            await Api.GetDocumentSingleMedia(_rows[i].MainPhoto.DocId, true, "image/jpeg").then((rslt) => {
                                _rows[i].VehicleMainPhotoData = rslt.values;
                            })
                                                                                                                            _rows[i].Name = _rows[i].Model?.Name || 'NA';
                                            }
                } else {
                    console.log(res.statusText);
                }
    ...
    const OnCloseClicked = async (e) => {
        if (e) {
            const rslt = await Api.SetVehicleSingle({ VehId: deletedId, Deleted: true });
            if (rslt.status) {
                setInitialize(true);
    ...
    const OnActionClicked = (id, type) => {
        let _route;
        if (type === 'edit') _route = `/Vehicles/edit/${id}`;
        if (type === 'view') _route = `/Vehicles/view/${id}`;
    ...
    return (
        ...
        <>
            <Container {...props}>
                <Box style={{ width: '100%', paddingBottom: 5 }}>
                    ...
                    <Stack direction="row">
                        <Grid container sx={{ justifyContent: 'flex-end' }}>
                                ...
                                onClick={() => NavigateTo("/Vehicles/create")}
                            >
                                <AddBoxIcon />
                            </IconButton>
                        </Grid>
                ...
                <Box style={{ width: '100%' }}>
                    <DataGrid keyId={'VehId'} rowsCount={rowsCount} rows={rows} sortBy={sortBy} pageInfo={pageInfo} onActionClicked={OnActionClicked}
