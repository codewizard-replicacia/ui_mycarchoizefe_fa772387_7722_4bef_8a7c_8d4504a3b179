        ...
        <>
            <GridContainer>
                {rows && rows.map((x, index) => (
                    <CardItem key={index} keyid={x.BookingId} title={x.DateOfBooking} imgsrc={x.Booking${imgProp}Data} width={300}
                        description={x.StartDateTime} onActionClicked={OnActionClicked}>
                    <Grid container direction="column">
                        <Typography variant="caption" color="text.secondary">
                            <strong>EndDateTime:</strong>&nbsp;{x.EndDateTime}
                        </Typography>
                        <Typography variant="caption" color="text.secondary">
                            <strong>TotalAmount:</strong>&nbsp;{x.TotalAmount}
                        </Typography>
                    </Grid>
                                                                                                                                                                                                                    </CardItem>
                ))}
