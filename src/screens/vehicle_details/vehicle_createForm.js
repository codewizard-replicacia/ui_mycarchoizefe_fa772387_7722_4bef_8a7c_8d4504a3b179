...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, prodImages, vehicleId;
        ...
        rslt = await Support.AddOrUpdateVehicle(vehicle, dropDownOptions, ['MainPhoto']);
        if (rslt.status) {
            vehicleId = rslt.id;
        } else { return; }

        for (let i = 0; i < mapItems.length; i++) {
            // Add or Update the Vehicle and navigation entity if it is deos not exist
            let navItem = vehicle.find(x => x.uicomponent === mapItems[i].uicomponent);
        ...
        global.AlertPopup("success", "Vehicle is created successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
