...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, changes = [], vehicleId, vehicle;

        vehicle = row['vehicle'];
        vehicleId = row['vehicle'].find((x) => x.type === 'keyid').value;
            ...
            if (changes.length > 0) {
                ...
                if (tmp.length > 0) {
                    ...
                    if (rslt.status) {
                        ...
                        if (Helper.IsNullValue(mapItem.navpropname)) vehicleId = rslt.id;
        ...
        // Add or Update Vehicle MainPhoto
        changes = TrackChanges('vehicle');
        if (changes.length > 0 && changes.indexOf('MainPhoto') > -1) {
            data = vehicle.find((x) => x.key === 'MainPhoto');
            rslt = await Support.AddOrUpdateDocument(data);
            if (rslt.status) {
                let newImageId = parseInt(rslt.id);
                data = [
                    { key: "VehId", value: parseInt(vehicleId) },
                    { key: "VehicleMainPhoto", value: newImageId }
                ];
                rslt = await Support.AddOrUpdateVehicle(data, dropDownOptions);
                if (!rslt.status) return;

                let newValues = vehicle.find((x) => x.key === 'MainPhoto').value;
                newValues = { ...newValues, DocId: newImageId };
                row['vehicle'].find((x) => x.key === 'MainPhoto').value = newValues;
                                // Update Back for next tracking purpose
                UpdateBackUp('vehicle');
        ...
        global.AlertPopup("success", "Vehicle is updated successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
