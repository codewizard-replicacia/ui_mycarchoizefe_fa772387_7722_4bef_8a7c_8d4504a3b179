...
const columns = [
    { headerName: "DateOfBooking", field: "DateOfBooking", flex: 1 },
    { headerName: "StartDateTime", field: "StartDateTime", flex: 1 },
    { headerName: "EndDateTime", field: "EndDateTime", flex: 1 },
    { headerName: "TotalAmount", field: "TotalAmount", flex: 1 },
    { headerName: "PickupAt", field: "LocationSimpleName", flex: 1 },
    { headerName: "Selected_Package", field: "Duration", flex: 1 },
];
...
const Component = (props) => {
    ...
    const FetchResults = async () => {
        ...
        await Api.GetBookingsMulti(query, "Selected_Package,PickupAt")
            .then(async (res) => {
                if (res.status) {
                    _rows = res.values || [];
                    if (_rows.length > 0) {
                        _rows.forEach(x => {
                            ...
                            x.LocationSimpleName = x.PickupAt?.LocationSimpleName || 'NA';


                            x.Duration = x.Selected_Package?.Duration || 'NA';
    ...
    const OnCloseClicked = async (e) => {
        if (e) {
            const rslt = await Api.SetBookingSingle({ BookingId: deletedId, Deleted: true });
            if (rslt.status) {
                setInitialize(true);
    ...
    const OnActionClicked = (id, type) => {
        let _route;
        if (type === 'edit') _route = `/Bookings/edit/${id}`;
        if (type === 'view') _route = `/Bookings/view/${id}`;
    ...
    return (
        ...
        <>
            <Container {...props}>
                ...
                <Box style={{ width: '100%' }}>
                    <DataTable keyId={'BookingId'} columns={columns} rowsCount={rowsCount} rows={rows} sortBy={sortBy} pageInfo={pageInfo} onActionClicked={OnActionClicked}
